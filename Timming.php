    ...
    public function index()
    {
        ...
        foreach($list as $k=>$v){
            ...
            if( $v['status']=='1' &&
                ...
                $file = $v['file'];
                $this->$file($v['param']);
                // 修复：每次任务后主动清理缓存和数据库连接，防止多任务状态污染
                if (class_exists('think\Cache')) { \think\Cache::clear(); }
                if (class_exists('think\Db') && method_exists('think\Db', 'clear')) { \think\Db::clear(); }
                if (isset($request)) { unset($request); }
                // 关键：清理Collect模型静态缓存，防止Query污染
                if (isset(\think\Model::$links['app\\common\\model\\Collect'])) {
                    unset(\think\Model::$links['app\\common\\model\\Collect']);
                }
                die;
            }
            else{
